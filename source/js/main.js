...
const sliderWindowTrainers = document.querySelector('.trainers__slider-window');
...
const sliderWindowAccounts = document.querySelector('.accounts__slider-window')
const accountsNextBtn = document.querySelector('.accounts__next-arrow-mobile');
const accountsPrevBtn = document.querySelector('.accounts__prev-arrow-mobile');


let slides = document.querySelectorAll('.trainers__trainer');
let photosNumber = 8;

const MOBILE_VISIBLE_WIDTH = 226;
const TABLET_VISIBLE_WIDTH = 566;
const DESKTOP_VISIBLE_WIDTH = 1160;

const TABLET_VISIBLE_PHOTOS = 2;
const DESKTOP_VISIBLE_PHOTOS = 4;

const MOBILE_SLIDES_NUMBER = photosNumber;
const TABLET_SLIDES_NUMBER = Math.ceil(photosNumber / TABLET_VISIBLE_PHOTOS);
const DESKTOP_SLIDES_NUMBER = Math.ceil(photosNumber / DESKTOP_VISIBLE_PHOTOS);

const TABLET_GAP = 30;
const DESKTOP_GAP = 40;

const TABLET_PHOTO_WIDTH = 268;
const DESKTOP_PHOTO_WIDTH = 260;

const MOBILE_SLIDER_WIDTH = MOBILE_SLIDES_NUMBER * MOBILE_VISIBLE_WIDTH;
const TABLET_SLIDER_WIDTH = TABLET_PHOTO_WIDTH * photosNumber + (TABLET_GAP * photosNumber) - TABLET_GAP;
const DESKTOP_SLIDER_WIDTH = DESKTOP_PHOTO_WIDTH * photosNumber + (DESKTOP_GAP * photosNumber) - DESKTOP_GAP;

const ANIMATION_DURATION_MOBILE = 200;
const ANIMATION_DURATION_TABLET = 400;
const ANIMATION_DURATION_DESKTOP = 500;

const ACCCOUNTS_MOBILE_VISIBLE_WIDTH = 226;

const accounts = document.querySelectorAll('.accounts__account');
const accountsNumber = accounts.length;

const ACCCOUNTS_MOBILE_SLIDER_WIDTH = 452;
const ACCCOUNTS_MOBILE_SLIDES_NUMBER = accountsNumber;
const ACCCOUNTS_ANIMATION_DURATION_MOBILE = 200;
...
const createTrainersTemplate = () => {
    return `<div class="trainers__slider">
                ...
                <div class="trainers__slider-window">
                    <div class="trainers__sequence trainers__sequence--visible">
                ...
                <div class="trainers__sequence trainers__sequence--hidden">
                    ${trainers.map((trainer, index) => {
                    let i = index + 1;
                        return `<div class="trainers__trainer trainers__trainer--${i}">
                            <div class="trainers__info-wrapper">
...
const trainersFeatures = {
    ...
    DESKTOP_GAP: 40,
    DESKTOP_SLIDER_WIDTH,
    DESKTOP_SLIDES_NUMBER,
...
const createAccountsTemplate = () => {
    ...
    <div class="accounts__slider-window">
        <div class="accounts__sequence accounts__sequence--visible">
        ...
        <div class="accounts__sequence accounts__sequence--hidden">
        ${accountsMocks.map((account) => {
           return `<div class="accounts__card">
                <h3 class="accounts__user-name">${account.name}</h3>
                <p class="accounts__account">${account.account}
...
const lessonsNumber = document.querySelector('.subscriptions__lessons-number');
...
const changeSubscription = (evt) => {
    ...
    if (!isSelected && months) {
        ...
        switch (months) {
            case MONTHS.ONE:
                coachPrice.textContent = COACH_PRICES.MONTHS1;
                dayPrice.textContent = DAY_PRICES.MONTHS1;
                fullDayPrice.textContent = FULL_DAY_PRICES.MONTHS1;
            ...
            case MONTHS.SIX:
                coachPrice.textContent = COACH_PRICES.MONTHS6;
                dayPrice.textContent = DAY_PRICES.MONTHS6;
                fullDayPrice.textContent = FULL_DAY_PRICES.MONTHS6;
            ...
            case MONTHS.TWELVE:
                coachPrice.textContent = COACH_PRICES.MONTHS12;
                dayPrice.textContent = DAY_PRICES.MONTHS12;
                fullDayPrice.textContent = FULL_DAY_PRICES.MONTHS12;
